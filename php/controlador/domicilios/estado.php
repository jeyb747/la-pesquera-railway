...
if (!in_array($_SESSION['rol'] ?? '', ['admin','repartidor'], true)) { header('Location: /version_final/index.php'); exit; }
$id=(int)($_POST['id']??0); $estado=$_POST['estado']??'';
if (!in_array($estado,['pendiente','en camino','entregado'],true)) { header('Location: /version_final/index.php'); exit; }
...
flash_set('success','Estado del domicilio actualizado.');header('Location: /version_final/paginas/'.(($_SESSION['rol']??'')==='admin'?'admin/domicilios.php':'repartidor/domicilios.php'));
